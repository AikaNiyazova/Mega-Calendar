...
//            "JOIN tb_meeting_dates md on m.id = md.meeting_id " +
            "WHERE mu.user_id = ?1 " +
            "AND m.meeting_date BETWEEN ?2 AND ?3", nativeQuery = true)
    List<Meeting> findAllByUserIdAndDates(Long userId, LocalDate startDate, LocalDate endDate);
...
//            "JOIN tb_meeting_dates md on m.id = md.meeting_id " +
            "WHERE m.room_id = ?1 " +
            "AND m.meeting_date BETWEEN ?2 AND ?3", nativeQuery = true)
    List<Meeting> findAllByRoomIdAndDates(Long roomId, LocalDate startDate, LocalDate endDate); //todo: Hidden Rooms?
